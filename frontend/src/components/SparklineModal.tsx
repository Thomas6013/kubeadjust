...
export default function SparklineModal({ isOpen, onClose, dataPoints, label, color, isCPU, title }: SparklineModalProps) {
  ...
  const xs = dataPoints.map((_, i) => PAD.left + (i / (dataPoints.length - 1)) * innerW);
  const ys = dataPoints.map((p) => PAD.top + (1 - (p.v - minV) / rangeV) * innerH);

  const pathD = xs.map((x, i) => `${i === 0 ? "M" : "L"}${x.toFixed(1)},${ys[i].toFixed(1)}`).join(" ");

  // Fill area under curve
  const fillD = `${pathD} L${xs[xs.length - 1].toFixed(1)},${(PAD.top + innerH).toFixed(1)} L${PAD.left.toFixed(1)},${(PAD.top + innerH).toFixed(1)} Z`;
    ...
    <div className={styles.overlay} onClick={onClose}>
      <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
        ...
        <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} className={styles.chart}>
          ...
          <text x={PAD.left + 2} y={PAD.top + innerH - 3} fontSize={9} fill="var(--muted)" fontFamily="monospace">
