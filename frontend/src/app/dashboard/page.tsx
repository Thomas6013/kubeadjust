...
export default function DashboardPage() {
  ...
  const loadDeployments = useCallback(async (ns: string) => {
    ...
    setError("");
    try {
      const deps = await api.deployments(token, ns);
      setDeployments(deps);
          ...
          {view === "nodes" ? (
            ...
            <>
              ...
              {loadingDeps ? (
                ...
                <p className={styles.muted}>No deployments in this namespace.</p>
              ) : (
                <div className={styles.depList}>
                  {deployments.map((dep) => <DeploymentCard key={dep.name} dep={dep} />)}
                </div>
              )}
            </>
          )}
