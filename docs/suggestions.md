...
| Group | Icon | Color | Contains |
|---|---|---|---|
| critical | ▲ | red | All `danger` items |
| warning | ● | orange | All `warning` items |
| over-prov | ▼ | blue | All `overkill` items |

Within each group, items are sorted by resource type (`RESOURCE_ORDER`). Each item displays a `resourceTag` badge showing the specific resource (e.g. `CPU — no limit`).

**Filter chips** (above the list) filter by resource category:

| Chip | Covers |
|---|---|
| CPU | `CPU`, `CPU — no limit`, `CPU — no request` |
| Memory | `Memory`, `Memory — no limit`, `Memory — no request` |
| Storage | `Ephemeral`, `Ephemeral — no limit`, `PVC`, `EmptyDir` |

Multiple chips can be active simultaneously (union filter). No chips active = show all.
